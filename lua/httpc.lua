...
-- ---@param node TSNode
-- ---@return nil
-- local run_request_node = function(buf, node)
--   local ns = vim.api.nvim_create_namespace("httpc-" .. tostring(buf))
--   local animation = get_opt("animation") --[[@as httpc.Animation]]
--   local idx = 0
--   local _, _, lnr = node:range()
--   local extmark = vim.api.nvim_buf_set_extmark(buf, ns, lnr, 0, {
...
--     idx = (idx + 1) % #animation
--     vim.api.nvim_buf_set_extmark(buf, ns, lnr, 0, {
--       id = extmark,
--       virt_lines_above = true,
--       virt_lines = {
--         animation[idx + 1],
...
--     if cnode:type() == "method" then
--       cmd[#cmd + 1] = "-X"
--       cmd[#cmd + 1] = vim.treesitter.get_node_text(cnode, buf)
--     elseif cnode:type() == "target_url" then
--       cmd[#cmd + 1] = vim.treesitter.get_node_text(cnode, buf)
--     elseif cnode:type() == "header" then
--       cmd[#cmd + 1] = "-H"
...
-- ---@param node TSNode
-- ---@return nil
-- local clear_request_node = function(buf, node)
--   local ns = vim.api.nvim_create_namespace("httpc-" .. tostring(buf))
--   local _, _, lnr = node:range()
--   for _, extmark in ipairs(vim.tbl_map(
--     function(e)
--       return e[1]
--     end,
--     vim.api.nvim_buf_get_extmarks(buf, ns, { lnr, 0 }, { lnr, 0 }, {})
--   )) do
--     local ctx = contexts[extmark]
--     if not ctx.process:is_closing() then
--       ctx.process:kill(9)
--     end
--     if not ctx.timer:is_closing() then
...
-- M.run = function(pos)
--   pos = get_pos(pos)
--   local node = get_request_node(pos)
--   if not node then
--     vim.notify("httpc: cannot get request node.", 4)
--     return
--   end
-- end
--
...
--   pos = get_pos(pos)
--   local request_node = get_request_node(pos)
--   if not request_node then
--     vim.notify("httpc: cannot get request node.", 4)
--     return
--   end
--   clear_request_node(pos.buf, request_node)
-- end
--
